...
abstract class BaseAccessLog implements ArrayAccess
{
  const
  ...
  public static function getFromRow(array $row)
  {
    ...
    $key = serialize($keys);
    if (!isset(self::$accessLogs[$key]))
    {
      $accessLog = new QubitAccessLog;
      ...
      self::$accessLogs[$key] = $accessLog;
    }

    return self::$accessLogs[$key];
  }

  public static function clearCache()
  {
    self::$accessLogs = array();
  }
  ...
  public static function get(Criteria $criteria, array $options = array())
  {
    ...
    {
      $options['connection'] = Propel::getConnection(QubitAccessLog::DATABASE_NAME);
    }

    self::addSelectColumns($criteria);

    return QubitQuery::createFromCriteria($criteria, 'QubitAccessLog', $options);
  }
  ...
  public static function getById($id, array $options = array())
  {
    ...
    $criteria->add(QubitAccessLog::ID, $id);
  ...
  {
    $this->tables[] = Propel::getDatabaseMap(QubitAccessLog::DATABASE_NAME)->getTable(QubitAccessLog::TABLE_NAME);
  }
  ...
  protected function rowOffsetGet($name, $offset, $options)
  {
    ...
    if (!array_key_exists($offset, $this->row))
    {
      ...
      $criteria = new Criteria;
      $criteria->add(QubitAccessLog::ID, $this->id);
  ...
  {
    $criteria->addJoin(QubitAccessLog::OBJECT_ID, QubitObject::ID);
